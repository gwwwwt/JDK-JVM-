...
> 源文件：src/hotspot/share/runtime/atomic.hpp

```c++
enum atomic_memory_order {
  // The modes that align with C++11 are intended to follow the same semantics.
  memory_order_relaxed = 0,
  memory_order_acquire = 2,
  memory_order_release = 3,
  memory_order_acq_rel = 4,
  memory_order_conservative = 8  // Strong two-way memory barrier.
};

/*
  对int64类型的原子操作不能在32位平台上使用. 也就是在使用本类中定义的int64原子操作
  之前, 最好确保当前是64位平台, 可以调用 supports_cx8() 来测试.
  
  本类中定义的每个原子函数族都来自 src/share/vim/runtime/orderAccess.hpp, 如
  <fence>语义就是由其中的OrderAccess::fence()函数实现的. 参考orderAccess.hpp
  来查看Memory Access Ordering Model细节.
...
> 以《启动流程/1. 入口.md》中的`3.3.1 JNI_CreateJavaVM_inner函数`一节为例。
>
> > **Atomic::xchg(T new_value, D* dest)函数的作用类似原子的执行如下的函数逻辑：**
> >
> > ```c++
> > T old_value = *dest;
> > *dest = T;
...
> > ```
> >
> > > **即：将dest指针指向的值设置为传入的新值，返回原来的旧值。**
> > >
> > > **所以要求模板方法参数：类型T和指针dest指向的类型D必须能够兼容。**

```c++
{
    //--- 下面两行比较好理解, 即原子的将a的值修改为0xdeadbeef, a的原旧值(即0xcafebabe)保存在b中,
    jint a = 0xcafebabe;
    jint b = Atomic::xchg((jint) 0xdeadbeef, &a);
  
    //--- 下面两行初次看可能有些疑问, 因为xchg方法的第一个参数传入了一个指针;
    //--- 但是可以简单的将指针值认为是一个整型数, 即xchg的第一个参数 &b 只是当做一个整数的话,
    //--- 所以结果就是 c的值设置为新值 &b, d中存储了c中原来的值;
    //--- 现时结合指针的意义, 即此时c中保存了b的地址, d中保存了原来c中的a的地址;
    void *c = &a;
