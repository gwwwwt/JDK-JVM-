...
)
{
    ...
    */
    ...
    // --- 10. 解析参数; 参考《6. ParseArguments解析参数.md》
...
> >
> > **所以执行到"JavaMain"函数时，_args中最主要的是封装了ifn(函数指针结构体)、mode(启动方式)、what(目录文件路径)。**
...
> **InitializeJVM()函数作用：初始化Java Virtual Machine，并且会在使用完options数组信息后释放options数组。关于options数组信息填充参考第2节和《6. ParseArguments解析参数.md》。**
>
