...
> **` Arguments::parse() 方法的目标就是，将上面3个scope所分别对应的JavaVMInitArgs统一到一个大的结果集中，这就是一个最终的完整配置信息。进而 Arguments就可以使用这一个完整的配置信息来设置Arguments中的对应在字段值。` **
>
> ***`即Arguments::parse()函数最终结果就反映到Arguments中的各个与配置相关的字段值上。`***
