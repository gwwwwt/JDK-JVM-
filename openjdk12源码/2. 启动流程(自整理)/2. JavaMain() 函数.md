...
>
> **参考《1. 命令行参数处理.md》，main()入口函数启动了一个新线程执行JavaMain函数，而命令行参数及其它信息则封装成了JavaMainArgs结构体作为JavaMain的参数传入。**
>
> > **注：其实JavaMainArgs结构体中已经不包含太多命令行信息了，因为经过JLI_Launch()函数中的`ParseArguments()`之后，相关的JVM选项，如"-cp", "-Xss"等配置，都已转换成了java.c#options数组中的相关配置。**
> >
